...
class RichTextToMarkdownConverterTest {
    ...
    @Test
    fun `renders feishu table nested directly inside ul as markdown table`() {
        val html = """
            <ul>
              <li>列表项文字
                <ul>
                  <div>
                    <table>
                      <tr><th>数据</th><th>计算公式</th></tr>
                      <tr><td>GMV</td><td>实付金额之和</td></tr>
                    </table>
                  </div>
                </ul>
              </li>
        ...
        assertTrue(markdown.contains("| 数据 | 计算公式 |"), "应当渲染表头")
        assertTrue(markdown.contains("| GMV | 实付金额之和 |"), "应当渲染表格数据行")
    }

    @Test
    fun `renders both list item text and nested table without losing content`() {
        val html = """
            <ul>
              <li>列表文字
                <ul>
        ...
        assertTrue(markdown.contains("列表文字"), "列表项文字不应丢失")
        assertTrue(markdown.contains("| 姓名 | 分数 |"), "表格不应丢失")
        assertTrue(markdown.indexOf("列表文字") < markdown.indexOf("| 姓名 |"), "文字应在表格之前")
    }

    @Test
    fun `ordered list sequence numbers are not affected by non-li children`() {
        val html = """
            <ol>
              <li>第一项</li>
              <div>非列表块</div>
        ...
        assertTrue(markdown.contains("1. 第一项"), "有序列表序号应从1开始")
        assertTrue(markdown.contains("2. 第二项"), "非li元素不应占用序号，第二项应为2")
    }

    @Test
    fun `renders table directly inside li as markdown table not as inline text`() {
        val html = """
            <ul>
              <li>
                <div>列表项</div>
        ...
        assertTrue(markdown.contains("| 字段 | 说明 |"), "table 直接在 li 内应渲染为 Markdown 表格，不应内联化")
        assertTrue(markdown.contains("| GMV | 实付金额 |"), "表格数据行应完整保留")
    }

    @Test
    fun `renders blockquote directly inside li with quote prefix`() {
        val html = """
            <ul>
              <li>
                <div>描述</div>
        ...
        assertTrue(markdown.contains("> 注意事项"), "blockquote 在 li 内应渲染为引用块，不应被压平为普通文字")
    }

    @Test
    fun `renders pre code directly inside li as fenced code block`() {
        val html = """
            <ul>
              <li>
                <div>示例</div>
        ...
        assertTrue(markdown.contains("```java"), "pre/code 在 li 内应渲染为 fenced code block")
        assertTrue(markdown.contains("System.out.println"), "代码内容应完整保留")
    }

    @Test
    fun `standard li-only list renders identically after fix`() {
        val html = """
            <ul>
              <li>苹果</li>
              <li>香蕉</li>
    ...
    @Test
    fun `renders nested list in table cell as html with preserved nesting`() {
        val html = """
            <table>
        ...
        assertTrue(markdown.contains("<li>项目A</li>"), "顶层列表项应用 html li 渲染")
        assertTrue(markdown.contains("<li>目标系统：<ul><li>子项1</li><li>子项2</li></ul></li>"), "嵌套子列表应保持 html 嵌套结构")
    }
    ...
    fun `renders rich text in table cells with html tags and br separators`() {
        ...
        assertTrue(markdown.contains("<strong>加粗</strong> + <em>强调</em><br/><a href=\"https://example.com\">链接</a><br/><ul><li>条目A</li><li>条目B</li></ul>"))
    }
    ...
    @Test
    fun `renders single-level unordered list in table cell as html ul`() {
        val html = """
            <table>
              <tr><th>项</th><th>内容</th></tr>
              <tr>
                <td>列表</td>
                <td><ul><li>苹果</li><li>香蕉</li></ul></td>
              </tr>
            </table>
        """.trimIndent()
        ...
        assertTrue(markdown.contains("<ul><li>苹果</li><li>香蕉</li></ul>"))
    }

    @Test
    fun `renders three-level nested unordered list in table cell preserving all levels`() {
        val html = """
            <table>
              <tr><th>项</th><th>内容</th></tr>
              <tr>
                <td>嵌套</td>
                <td><ul><li>一级<ul><li>二级<ul><li>三级</li></ul></li></ul></li></ul></td>
              </tr>
            </table>
        """.trimIndent()
        ...
        assertTrue(markdown.contains("<ul><li>一级<ul><li>二级<ul><li>三级</li></ul></li></ul></li></ul>"))
    }

    @Test
    fun `renders ordered list in table cell as html ol`() {
        val html = """
            <table>
              <tr><th>项</th><th>内容</th></tr>
              <tr>
        ...
        assertTrue(markdown.contains("<ol><li>第一步</li><li>第二步</li></ol>"))
    }

    @Test
    fun `renders strong inside table cell list item as html strong tag`() {
        val html = """
            <table>
              <tr><th>项</th><th>内容</th></tr>
              <tr>
                <td>格式</td>
                <td><ul><li><strong>加粗内容</strong></li></ul></td>
              </tr>
            </table>
        """.trimIndent()
        ...
        assertTrue(markdown.contains("<li><strong>加粗内容</strong></li>"))
    }
}
